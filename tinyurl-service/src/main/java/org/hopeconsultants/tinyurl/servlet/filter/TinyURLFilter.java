...
@Component(
	...
	property = {
		...
		"servlet-filter-name=Tiny URL Filter",
		"url-pattern=/t/*",
	},
...
public class TinyURLFilter extends BaseFilter {
	...
	protected void processFilter(
		...
		if (_configuration.secureDecode()) {
		...
		try {
			...
			processFilter(
				TinyURLFilter.class.getName(), request, response, filterChain);

			return;
		}

		TinyURL tinyURL = _tinyURLService.fetchTinyURL(tinyURLId);
		...
		if (classNameId == TinyURLConstants.CLASSNAMEID_DLFILEENTRY) {
			...
			try {
				...
				processFilter(
					TinyURLFilter.class.getName(), request, response,
					filterChain);
	...
	@Reference(unbind = "-")
	private TinyURLLocalService _tinyURLService;
