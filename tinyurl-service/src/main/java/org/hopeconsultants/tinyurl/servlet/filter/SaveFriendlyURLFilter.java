...
public class SaveFriendlyURLFilter extends BaseFilter {
    ...
    public boolean isFilterEnabled(
        ...
        return (!_isAlreadyFiltered(request) &&
            (request.getAttribute(TinyURLTools.FRIENDLY_URL) == null));
    }
    ...
    protected void processFilter(
	    ...
	    request.setAttribute(
            TinyURLTools.FRIENDLY_URL, _getFriendlyURL(pathInfo));

        processFilter(
            SaveFriendlyURLFilter.class.getName(), request, response,
            filterChain);
    }
